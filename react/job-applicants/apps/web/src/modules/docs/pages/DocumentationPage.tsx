...
export default function DocumentationPage() {
    const { t } = useTranslation('docs');
    const uiItems = t('ui', { returnObjects: true }) as string[];
    const principles = ['separation', 'source', 'automation', 'modularity', 'extraction', 'readability'] as const;
    return (
        <main className="min-h-screen bg-background text-foreground">
            <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
                <header className="border-b pb-10">
                    <p className="mb-3 text-sm font-medium text-primary">{t('projectDocumentation')}</p>
                ...
                <nav aria-label="Breadcrumb" className="py-6 text-sm text-muted-foreground">
                    <ol className="flex flex-wrap items-center gap-2">
                        <li><a href="/" className="hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">{t('home')}</a></li>
                        <li aria-hidden="true">/</li>
                        <li aria-current="page" className="text-foreground">{t('documentation')}</li>
                    </ol>
                </nav>
                ...
                <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_220px]">
                    ...
                    <article className="min-w-0 max-w-3xl space-y-14 leading-7 lg:col-start-1 lg:row-start-1">
                        ...
                        <section className="scroll-mt-24">
                            <h2 id="the-application" className="text-2xl font-semibold tracking-tight">{t('theApplication')}</h2>
                            <h3 id="deployment" className="mt-8 text-xl font-semibold">{t('deployment')}</h3>
                            <div className="mt-3 text-muted-foreground">
                                <p>{t('deploymentFirst')}</p>
                                <p>{t('deploymentSecond')}</p>
                            </div>
                            <h3 id="user-interface" className="mt-8 text-xl font-semibold">{t('userInterface')}</h3>
                            <ul className="mt-3 list-disc space-y-1 ps-5 text-muted-foreground">
                                {uiItems.map((item) => <li key={item}>{item}</li>)}
                            </ul>
                        </section>
                        <section className="scroll-mt-24">
                            <h2 id="the-code" className="text-2xl font-semibold tracking-tight">{t('theCode')}</h2>
                            <h3 id="code-policy" className="mt-8 text-xl font-semibold">{t('codePolicy')}</h3>
                            {principles.map((key) => <div key={key}><h4 className="mt-6 text-lg font-semibold">{t(`principles.${key}.title`)}</h4><ul className="mt-2 list-disc space-y-1 ps-5 text-muted-foreground">{(t(`principles.${key}.items`, { returnObjects: true }) as string[]).map((item) => <li key={item}>{item}</li>)}</ul></div>)}
                            <div className="mt-8 flex flex-wrap items-center justify-between gap-3">
                                <h3 id="repository-structure" className="text-xl font-semibold">{t('repositoryStructure')}</h3>
                                <a href={repositoryUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-sm font-medium text-primary underline-offset-4 hover:bg-muted hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                                    {t('viewOnGithub')} <ExternalLink aria-hidden="true" className="size-3.5" />
                                </a>
                            </div>
                            <RepositoryTree />
                            <h3 id="known-limitations" className="mt-8 text-xl font-semibold">{t('knownLimitations')}</h3>
                            <div className="mt-4 flex gap-3 rounded-lg border border-primary/20 bg-primary/5 p-4 dark:bg-primary/10">
                                ...
                                <ul className="list-disc space-y-2 ps-5 text-muted-foreground">
                                    {(t('limitations', { returnObjects: true }) as string[]).map((item) => <li key={item}>{item}</li>)}
                                </ul>
                            </div>
                        </section>
                        <section className="scroll-mt-24">
                            <h2 id="case-study" className="text-2xl font-semibold tracking-tight">{t('caseStudy')}</h2>
                            <h3 id="evolution" className="mt-8 text-xl font-semibold">{t('evolution')}</h3>
                            <EvolutionTimeline />
                        </section>
                    </article>
                </div>
